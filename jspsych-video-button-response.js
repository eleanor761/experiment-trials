  ...
  const info = {
    ...
    grid_columns: {
      ...
      feedback_duration: {
        ...
        default: 400,
        description: 'How long to show the feedback before moving to next trial'
      }
    }
  };
  ...
  class VideoButtonResponsePlugin {
    ...
    trial(display_element, trial) {
      ...
      leftButton.addEventListener('click', function() {
        showFeedback(0, leftButton);
      });
      rightButton.addEventListener('click', function() {
        showFeedback(1, rightButton);
      });

      // Function to show feedback then end trial
      function showFeedback(buttonIndex, clickedButton) {
        // Disable both buttons to prevent multiple clicks
        leftButton.disabled = true;
        rightButton.disabled = true;
        
        // Calculate RT immediately
        let reaction_time = null;
        if (start_time !== null) {
          reaction_time = performance.now() - start_time;
        }
        
        // Determine if response is correct (if data.word exists in trial object)
        let isCorrect = false;
        if (trial.data && trial.data.word && trial.choices) {
          const selectedChoice = trial.choices[buttonIndex];
          isCorrect = (selectedChoice === trial.data.word);
        }
        
        // Apply feedback styling
        if (isCorrect) {
          clickedButton.style.border = '3px solid #22c55e'; // Green border
          clickedButton.style.backgroundColor = '#dcfce7'; // Light green background
          new Audio('stimuli/bleep.wav').play();
        } else {
          clickedButton.style.border = '3px solid #ef4444'; // Red border
          clickedButton.style.backgroundColor = '#fee2e2'; // Light red background
          new Audio('stimuli/buzz.wav').play();
        }
        
        // Wait for feedback duration then end trial
        setTimeout(function() {
          end_trial(buttonIndex, reaction_time);
